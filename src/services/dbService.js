...
export const registerPayment = async (storeId, months = 1, value = 0) => {
    try {
        ...
        // --- INÍCIO TRAMITE COMISSÃO DE REVENDEDOR ---
        if (storeData.partnerId) {
             try {
                 const partnerRef = doc(db, "partners", storeData.partnerId);
                 const partnerSnap = await getDoc(partnerRef);
                 if (partnerSnap.exists()) {
                      const partnerData = partnerSnap.data();
                      const commissionRate = parseFloat(partnerData.commissionRate) || 0; 
                      
                      if (commissionRate > 0 && value > 0) {
                          const commissionValue = (value * (commissionRate / 100));
                          const commissionData = {
                              partnerId: partnerData.id || storeData.partnerId,
                              partnerName: partnerData.name || 'Desconhecido',
                              storeId: storeId,
                              storeName: storeData.name,
                              paymentValue: value,
                              commissionRate: commissionRate,
                              commissionValue: commissionValue,
                              status: 'PENDING', // Agendado para pagamento ao revendedor
                              createdAt: new Date().toISOString(),
                              monthsPaid: months
                          };
                          await addDoc(collection(db, "partner_commissions"), commissionData);
                      }
                 }
             } catch(partnerErr) {
                 console.error("Erro ao processar comissão do revendedor:", partnerErr);
                 // Não trava a renovação da loja caso falhe a comissão
             }
        }
        // --- FIM TRAMITE COMISSÃO ---
...
export const addPartner = async (partnerData) => {
    try {
        const data = { ...partnerData, createdAt: new Date().toISOString() };
...
export const getPartnerCommissions = async () => {
    try {
        const q = query(collection(db, "partner_commissions"), orderBy("createdAt", "desc"));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    } catch (error) {
        console.error("Error fetching commissions:", error);
        try {
            const querySnapshot = await getDocs(collection(db, "partner_commissions"));
...
export const updateCommissionStatus = async (id, status) => {
    try {
        const docRef = doc(db, "partner_commissions", id);
        await updateDoc(docRef, { status, paidAt: status === 'PAID' ? new Date().toISOString() : null });
        return { success: true };
    } catch (error) {
        return { success: false, error: error.message };
    }
