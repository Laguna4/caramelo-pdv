export const printBudget = (shopInfo, customerInfo, budgetItems, budgetTotal, budgetDate, printerSettings, observation = '', discount = 0, subtotal = 0) => {
                    ...
                    {discount > 0 && (
                        <>
                            <tr className="total-row">
                                <td colspan="2" style="text-align: right; padding-top: 10px; font-size: 10px; color: #666;">SUBTOTAL:</td>
                                <td style="text-align: right; padding-top: 10px; font-size: 10px; color: #666;">${formatCurrency(subtotal || budgetTotal + discount)}</td>
                            </tr>
                            <tr className="total-row">
                                <td colspan="2" style="text-align: right; color: #d00; font-size: 10px;">DESCONTO:</td>
                                <td style="text-align: right; color: #d00; font-size: 10px;">-${formatCurrency(discount)}</td>
                            </tr>
                        </>
                    )}
                    <tr class="total-row">
                        <td colspan="2" style="text-align: right; padding-top: 10px; font-weight: 900;">TOTAL GERAL:</td>
                        <td style="text-align: right; padding-top: 10px; font-weight: 900; font-size: 16px;">${formatCurrency(budgetTotal)}</td>
                    </tr>
                </tfoot>
            </table>

            ${observation ? `
                <div class="section" style="background: #fff9ed; border-color: #ffedc2;">
                    <div class="section-title">OBSERVAÇÕES</div>
                    <div style="font-size: 11px; white-space: pre-wrap;">${observation}</div>
                </div>
            ` : ''}
