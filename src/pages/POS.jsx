...
const POS = () => {
    ...
    const handleSaveBudget = async (budgetInfo = {}) => {
        ...
        const { discount = 0, observation = '' } = budgetInfo;
                ...
                <form onSubmit={handleScan} className="barcode-input relative z-50">
                    <div className="flex gap-2">
                        <div className="flex flex-col relative group">
                            <span className="text-[10px] text-gray-500 font-bold uppercase mb-1 whitespace-nowrap hidden md:block">Metros/Qtd</span>
                            <input
                                type="number"
                                step="any"
                                value={inputQuantity}
                                onChange={(e) => setInputQuantity(e.target.value)}
                                className="w-16 md:w-20 bg-[#111] border-2 border-gray-800 text-white text-center rounded-xl focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all font-bold text-sm md:text-lg h-12 md:h-14"
                                min="0.01"
                                placeholder="1"
                                disabled={!cashRegister}
                                title="Quantidade (Ex: 7 para Metros, ou 1.5 para Kilos)"
                            />
                        </div>
                        <div className="relative flex-1 group md:mt-[20px]">
                            <div className="absolute inset-y-0 left-0 pl-3 md:pl-4 flex items-center pointer-events-none">
                                <FaBarcode className="text-gray-500 group-focus-within:text-orange-500 transition-colors" size={18} />
                            </div>
                            <input
                                ref={barcodeInputRef}
                                className="w-full bg-[#111] border-2 border-gray-800 text-white text-base md:text-lg rounded-xl pl-10 md:pl-12 pr-4 h-12 md:h-14 focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all placeholder-gray-600"
                        ...
                        <div className="flex gap-1 md:mt-[20px]">
